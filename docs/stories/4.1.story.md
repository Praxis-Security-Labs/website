...
## Tasks / Subtasks
- [ ] Task 1: Complete Astro i18n Framework Implementation (AC: 1, 4) - **PREREQUISITE FOR ALL OTHER TASKS**
  - [ ] Implement Astro i18n configuration in `astro.config.mjs` with English and Norwegian locales
  - [ ] Create language routing structure: `/` for English, `/no/` for Norwegian content
  - [ ] Configure URL rewriting and language detection middleware
  - [ ] Implement language parameter extraction in BaseLayout for proper routing
  - [ ] Add language-specific sitemap generation for both English and Norwegian pages
  - [ ] Configure hreflang tags in BaseLayout for proper language targeting
  - [ ] Test language URL routing works correctly across all existing pages
  - [ ] Ensure language switching preserves current page context and URL parameters

- [ ] Task 2: Content Translation Infrastructure (AC: 2, 3) - **REQUIRES Task 1**
  - [ ] Create `src/i18n/` directory structure for translation files
  - [ ] Implement translation utility functions and hooks for component usage
...
- [ ] Task 3: Language Switcher Component Enhancement (AC: 5) - **REQUIRES Task 1**
  - [ ] Update `src/components/layout/Header.tsx` language switcher with improved UX
  - [ ] Implement persistent language preference storage using Zustand store
  - [ ] Add conversion state preservation across language changes
  - [ ] Include proper ARIA labels and accessibility for language switching
  - [ ] Add visual indicators for current language selection
  - [ ] Implement smooth transitions and loading states for language switching
  - [ ] Test language switcher maintains form data and scroll position
  - [ ] Ensure language preference persists across browser sessions

- [ ] Task 4: Norwegian Page Creation and Routing (AC: 4) - **REQUIRES Tasks 1, 2**
  - [ ] Create `/src/pages/no/` directory structure mirroring English pages
  - [ ] Implement Norwegian homepage at `/src/pages/no/index.astro`
  - [ ] Create Norwegian product pages under `/src/pages/no/product/`
  - [ ] Create Norwegian segment pages under `/src/pages/no/segments/`
  - [ ] Create Norwegian about page under `/src/pages/no/about/`
  - [ ] Create Norwegian resources pages under `/src/pages/no/resources/`
  - [ ] Create Norwegian pricing page at `/src/pages/no/pricing.astro`
  - [ ] Create Norwegian contact page at `/src/pages/no/contact.astro`
  - [ ] Implement Norwegian legal pages under `/src/pages/no/legal/`
  - [ ] Configure proper page metadata and SEO for Norwegian pages

- [ ] Task 5: Localized SEO and Meta Tags Implementation (AC: 6) - **REQUIRES Tasks 1, 4**
  - [ ] Update BaseLayout.astro with Norwegian-specific meta tags
  - [ ] Implement Norwegian-optimized title tags for all pages
  - [ ] Create Norwegian meta descriptions maintaining SEO keyword targeting
  - [ ] Add Norwegian alt text for all images and graphics
  - [ ] Implement Norwegian-specific Open Graph and Twitter Card metadata
  - [ ] Add Norwegian keywords targeting security culture and behavioral monitoring terms
  - [ ] Configure Norwegian-specific structured data markup (JSON-LD)
...
- [ ] Task 8: Navigation and Menu Translation (AC: 2, 5) - **REQUIRES Tasks 1, 2**
  - [ ] Translate all navigation menu items and labels to Norwegian
  - [ ] Update footer links and legal navigation to Norwegian
  - [ ] Translate breadcrumb navigation components to Norwegian
  - [ ] Implement Norwegian-specific navigation structure if needed for cultural adaptation
  - [ ] Add Norwegian tooltips and help text for interactive elements
  - [ ] Translate call-to-action buttons with Norwegian persuasive language
  - [ ] Update search functionality to support Norwegian content
  - [ ] Test navigation works correctly across both language versions
